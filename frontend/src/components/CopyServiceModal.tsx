...
function CopyServiceModal({ service, onClose, onSave }: CopyServiceModalProps) {
  const [formData, setFormData] = useState({
    ...
    verify_ssl: service.verify_ssl === true || (service.verify_ssl as unknown) === 1,
    ssl_expiry_threshold: service.ssl_expiry_threshold || 30,
    verify_domain: service.verify_domain === true || (service.verify_domain as unknown) === 1,
    retry_count: service.retry_count ?? 3,
    retry_delay: service.retry_delay ?? 5,
  });
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-900">Copy to New Service</h2>
          <p className="text-sm text-gray-500 mt-1">
            Review and adjust the fields below before saving this as a new service.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4">
          <div className="space-y-6">
            ...
            {/* SSL & Domain Verification Section */}
            <div className="border-t pt-4">
              <h3 className="text-sm font-semibold text-gray-900 mb-4">SSL & Domain verification</h3>
              ...
              <div className="space-y-4">
                ...
                {formData.verify_ssl && (
                  <div className="ml-6">
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      SSL expiry alert threshold (days)
                    </label>
                    <input
                      type="number"
                      value={formData.ssl_expiry_threshold}
                      onChange={(e) =>
                        setFormData({ ...formData, ssl_expiry_threshold: parseInt(e.target.value) || 30 })
                      }
                    ...
                    />
                    <p className="text-xs text-gray-500 mt-1">
                      Alert when the SSL certificate expires within this many days. Checks certificate validity, issuer, and expiration date.
                    </p>
                  </div>
                )}
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={formData.verify_domain}
                    onChange={(e) => setFormData({ ...formData, verify_domain: e.target.checked })}
                    className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                  />
                  <span className="text-sm text-gray-700">Verify domain DNS resolution</span>
                </label>
                {formData.verify_domain && (
                  <p className="text-xs text-gray-500 ml-6">
                    Checks that the domain resolves via DNS (A/AAAA records). Alerts if the domain fails to resolve.
                  </p>
                )}
              </div>
            </div>
